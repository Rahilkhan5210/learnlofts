...
const MEDIA_API = `${import.meta.env.VITE_API_URL || 'https://learnlofts.onrender.com'}/api/v1/media`;
...
const LectureTab = () => {
  ...
  const editLectureHandler = async () => {
    ...
    try {
      const result = await edtiLecture({
        lectureTitle,
        videoInfo: uploadVideoInfo,
        isPreviewFree: isFree,
        courseId,
      ...
      console.log("Edit lecture response:", result);

      if (result.error) {
        toast.error(result.error.data?.message || "Failed to update lecture");
      } else {
  ...
  return (
    <Card>
      ...
      <CardContent>
        ...
        <div className="mt-4">
          <Button 
            disabled={isLoading || (!uploadVideoInfo && !lecture?.videoUrl)} 
